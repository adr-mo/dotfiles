...
curl -s --request GET "https://centreon.atlassian.net/rest/api/2/project/BAM/version?maxResults=200" \
    --header 'Authorization: Basic YW1vcmFpc0BjZW50cmVvbi5jb206WnBVSjZ5V055STlrZ1FoQlZsTkUyMTBB==' \
    --header 'Content-Type: application/json' | jq ".values[] | [.name, .released] | @csv" | tr -d \" | tr -d \\ > releases-bam.csv


while IFS=, read -r name released
do
    create_release "$name" "$released"
done < releases-bam.csv